// g++ -m32 -fPIC -shared dlshook.cc utils.cc dog_server.cc card_emulator.c -std=c++17 -o dlshook.so -ldl -lstdc++fs
// for building with docker:
// docker pull ckdur/manylinux-glibc212-32bit
// docker run --rm -it -v $(pwd):/app ckdur/manylinux-glibc212-32bit /bin/bash
#ifndef _GNU_SOURCE
#define _GNU_SOURCE
#endif
#include <cstdio>
...
static int (*real_tcgetattr)(int, struct termios*) = (int (*)(int, struct termios*))dlsym(RTLD_NEXT, "tcgetattr");
static int (*real_tcsetattr)(int, int, const struct termios*) = (int (*)(int, int, const struct termios*))dlsym(RTLD_NEXT, "tcsetattr");
static int (*real_tcflush)(int, int) = (int (*)(int, int))dlsym(RTLD_NEXT, "tcflush");
...
extern "C" int tcgetattr(int fd, struct termios *termios_p) {
    ...
    if (real_tcgetattr == nullptr) {
        real_tcgetattr = (int (*)(int, struct termios*))dlsym(RTLD_NEXT, "tcgetattr");
    }
...
extern "C" int tcsetattr(int fd, int optional_actions, const struct termios *termios_p) {
    int result = -1;

    if (real_tcsetattr == nullptr) {
        real_tcsetattr = (int (*)(int, int, const struct termios*))dlsym(RTLD_NEXT, "tcsetattr");
    }
...
static void unprotect(void *addr, size_t size) {
    ...
    mprotect((void*)((int)addr-((int)addr%size)),size,PROT_READ|PROT_WRITE|PROT_EXEC);
}
