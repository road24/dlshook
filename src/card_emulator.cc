...
bool CardEmulator::writeSector(const std::vector<uint8_t>& data, uint8_t sector) {
...
uint32_t CardEmulator::getId() const {
    return mId;
}

void CardEmulator::setId(uint32_t id) {
    mId = id;
}
...
void CardEmulator::processCommand(uint8_t command, uint8_t arg, const std::vector<uint8_t>& data) {
...
bool CardEmulator::onReadCardMetadata() {
    ...
    mRxQueue.push(0x0A); // Length byte (12 bytes for header + 1 byte for command + 1 byte for checksum)
    mRxQueue.push(static_cast<uint8_t>(card_emulator_cmd::CMD_READ_CARD_METADATA)); // Command byte
...
bool CardEmulator::onReadSector(uint8_t sector) {
    ...
    mRxQueue.push(0x02); // Start byte
    mRxQueue.push(0x34); // Length byte (2 bytes for header + 1 byte for command + 1 byte for checksum)
    mRxQueue.push(static_cast<uint8_t>(card_emulator_cmd::CMD_READ_SECTOR)); // Command byte
...
bool CardEmulator::onWriteSector(const std::vector<uint8_t>& data, uint8_t sector) {
    ...
    mRxQueue.push(0x02); // Start byte
    mRxQueue.push(0x05); // Length byte (5 bytes for header + 1 byte for command + 1 byte for checksum)
    mRxQueue.push(static_cast<uint8_t>(card_emulator_cmd::CMD_WRITE_SECTOR)); // Command byte
    mRxQueue.push(sector); // Sector number
    // Push the "ack" byte
